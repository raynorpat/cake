...
void Sys_Error (char *error, ...)
{
	...
	if (cl_hwnd)
		MessageBox (cl_hwnd, text, "Error", 0 /* MB_OK */);
	else MessageBox (NULL, text, "Error", 0 /* MB_OK */);
	...
	while (1)
	{
		...
		while (PeekMessage (&msg, NULL, 0, 0, PM_NOREMOVE))
		{
			...
			if (!ret)
				...
				Sys_Error ("WinMain : GetMessage failed");
			else
			{
				TranslateMessage (&msg);
				DispatchMessage (&msg);
		...
		// continue, don't spin, so that the message pump keeps pumping
		// hey! you can do this!
		if ((time = (newtime = Sys_Milliseconds ()) - oldtime) > 0)
		{
