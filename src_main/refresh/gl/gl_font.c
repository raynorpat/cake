...
// credit in the credits ) and then saved off the glyph data and then hand touch up the
...
static FT_Bitmap *R_RenderGlyph(FT_GlyphSlot glyph, glyphInfo_t * glyphOut)
{
	...
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "Non-outline fonts are not supported\n");
	}
	return NULL;
}
...
static glyphInfo_t *RE_ConstructGlyphInfo(unsigned char *imageOut, int *xOut, int *yOut, int *maxHeight, FT_Face face, const unsigned char c, qboolean calcHeight)
{
	...
	if(face != NULL)
	{
		...
		if(*xOut + scaledWidth + 1 >= (FONT_SIZE - 1))
		{
			...
			{
				//VID_Printf(PRINT_ALL, S_COLOR_RED "RE_ConstructGlyphInfo: character %c does not fit width and height\n", c);
			...
			{
				//VID_Printf(PRINT_ALL, S_COLOR_RED "RE_ConstructGlyphInfo: character %c does not fit width\n", c);
		...
		{
			//VID_Printf(PRINT_ALL, S_COLOR_RED "RE_ConstructGlyphInfo: character %c does not fit height\n", c);
...
void RE_GL_RegisterFont (char *fontName, int pointSize, fontInfo_t *font)
{
	...
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: Too many fonts registered already.\n");
		return;
	}

	COM_StripExtension (fontName, strippedName);
...
#ifndef USE_FREETYPE
	VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: FreeType code not available\n");
#else
	if(ftLibrary == NULL)
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: FreeType not initialized.\n");
		return;
	}

	Com_sprintf(fileName, sizeof(fileName), "%s.ttf", strippedName);
	len = FS_LoadFile(fileName, &faceData);
	if(len <= 0)
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: Unable to read font file '%s'\n", fileName);
		return;
	}
	...
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: FreeType2, unable to allocate new face.\n");
		return;
	}

	if(FT_Set_Char_Size(face, pointSize << 6, pointSize << 6, dpi, dpi))
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: FreeType2, Unable to set face char size.\n");
		return;
	}
	...
	if(out == NULL)
	{
		VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: malloc failure during output image creation.\n");
		return;
	}
	...
	while(i <= GLYPH_END)
	{
		...
		if(xOut == -1 || yOut == -1 || i == GLYPH_END)
		{
			//if(xOut == -1 || yOut == -1)
			//{
			//	VID_Printf(PRINT_ALL, S_COLOR_RED "RE_RegisterFont: character %c does not fit image number %i\n", (unsigned char) i, imageNumber);
			//}
			...
			{
				char fileNameWithPath[MAX_QPATH];
				Q_strlcpy(fileNameWithPath, fileName, sizeof(fileName));
				stbi_write_png(va("%s/%s", FS_Gamedir(), fileNameWithPath), FONT_SIZE, FONT_SIZE, 4, imageBuff, 0);
			}

			image = GL_LoadPic(fileName, imageBuff, FONT_SIZE, FONT_SIZE, it_pic, 32);
			...
			if(i == GLYPH_END)
			{
				for(j = lastStart; j <= GLYPH_END; j++)
					Q_strlcpy (font->glyphs[j].imageName, fileName, sizeof(font->glyphs[j].imageName));
	...
	Com_sprintf(fileName, sizeof(fileName), "%s_%i.dat", strippedName, pointSize);
	Q_strlcpy(font->name, fileName, sizeof(font->name));
	if (!FS_FileExists(fileName))
	{
		FS_FOpenFile(fileName, &f, FS_WRITE, true);
		FS_Write(font, sizeof(fontInfo_t), f);
		FS_FCloseFile(f);
	}
...
void R_InitFreeType (void)
{
	...
	if(FT_Init_FreeType(&ftLibrary))
		VID_Printf(PRINT_ALL, S_COLOR_RED "R_InitFreeType: Unable to initialize FreeType.\n");
#endif
