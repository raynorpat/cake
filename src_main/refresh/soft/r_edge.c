...
void D_BackgroundSurf (surf_t *s)
{
	...
	D_FlatFillSurface (s, (int)((intptr_t)sw_clearcolor->value) & 0xFF);
...
void D_TurbulentSurf (surf_t *s)
{
	...
	if (s->insubmodel)
	{
		...
		currententity = s->entity;	//FIXME: make this passed in to
									// R_RotateBmodel ()
		VectorSubtract (r_origin, currententity->currorigin,
				local_modelorg);
...
void D_SolidSurf (surf_t *s)
{
	...
	if (s->insubmodel)
	{
		...
		currententity = s->entity;	// FIXME: make this passed in to
						// R_RotateBmodel ()
		VectorSubtract (r_origin, currententity->currorigin, local_modelorg);
		TransformVector (local_modelorg, transformed_modelorg);
