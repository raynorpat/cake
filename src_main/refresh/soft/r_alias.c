...
float sw_avertexnormals[NUMVERTEXNORMALS][3] = {
#include "anorms.h"
};
...
void R_AliasSetUpTransform (void)
{
	...
	aliasworldtransform[0][3] = currententity->currorigin[0]-r_origin[0];
	aliasworldtransform[1][3] = currententity->currorigin[1]-r_origin[1];
	aliasworldtransform[2][3] = currententity->currorigin[2]-r_origin[2];

	aliasoldworldtransform[0][3] = currententity->lastorigin[0]-r_origin[0];
	aliasoldworldtransform[1][3] = currententity->lastorigin[1]-r_origin[1];
	aliasoldworldtransform[2][3] = currententity->lastorigin[2]-r_origin[2];
	...
	aliasworldtransform[0][3] = currententity->currorigin[0];
	aliasworldtransform[1][3] = currententity->currorigin[1];
	aliasworldtransform[2][3] = currententity->currorigin[2];

	aliasoldworldtransform[0][3] = currententity->lastorigin[0];
	aliasoldworldtransform[1][3] = currententity->lastorigin[1];
	aliasoldworldtransform[2][3] = currententity->lastorigin[2];
}
...
void R_AliasTransformFinalVerts( int numpoints, finalvert_t *fv, dtrivertx_t *oldv, dtrivertx_t *newv )
{
	...
	for ( i = 0; i < numpoints; i++, fv++, oldv++, newv++ )
	{
		...
		plightnormal = sw_avertexnormals[newv->lightnormalindex];
...
void R_AliasSetupLighting (void)
{
	...
	{
		SW_R_LightPoint (currententity->currorigin, light);
	}
...
{
	int thisframe = currententity->currframe;
	int lastframe = currententity->lastframe;
...
void R_AliasSetUpLerpData( dmdl_t *pmdl, float backlerp )
{
	...
	*/
	VectorSubtract (currententity->lastorigin, currententity->currorigin, translation);
