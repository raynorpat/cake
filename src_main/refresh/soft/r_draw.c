...
================
*/
image_t *RE_SW_Draw_RegisterPic(char *name)
{
	image_t	*image;
...
=============
*/
void RE_SW_Draw_GetPicSize(int *w, int *h, char *pic)
{
	image_t *gl;

	gl = RE_SW_Draw_RegisterPic (pic);
	if (!gl)
	{
		*w = *h = -1;
		return;
...
=============
*/
static void Draw_StretchPicImplementation (int x, int y, int w, int h, image_t	*pic)
{
	pixel_t	*dest;
	byte	*source;
	int		v, u;
...
void RE_SW_Draw_StretchPic(int x, int y, int w, int h, char *name)
{
	...
	pic = RE_SW_Draw_RegisterPic (name);
	if (!pic)
	{
...
void RE_SW_Draw_PicScaled(int x, int y, char *name, float scale)
{
	...
	{
		Com_Printf( "Draw_Pic: bad coordinates\n");
		return;
	}
