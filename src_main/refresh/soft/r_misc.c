...
void D_ViewChanged (void)
{
	...
	d_pix_max = (int)((float)r_refdef.vrect.width / (800.0 / 4.0) + 0.5);
	d_pix_shift = 8 - (int)((float)r_refdef.vrect.width / 800.0 + 0.5);
	if (d_pix_max < 1)
		d_pix_max = 1;
	...
	*/
	if ( r_newrefdef.rdflags & RDF_NOWORLDMODEL )
	{
		...
		RE_SW_Draw_Fill( r_newrefdef.x, r_newrefdef.y, r_newrefdef.width, r_newrefdef.height,( int ) sw_clearcolor->value & 0xff );
	}
...
void R_SetupFrame (void)
{
	...
	if ( !( r_newrefdef.rdflags & RDF_NOWORLDMODEL ) )
	{
		r_viewleaf = SW_Mod_PointInLeaf (r_origin, r_worldmodel);
