...
void SW_Mod_LoadSpriteModel (model_t *mod, void *buffer);
void SW_Mod_LoadBrushModel (model_t *mod, void *buffer);
void SW_Mod_LoadAliasModel (model_t *mod, void *buffer);
...
==================
*/
model_t *SW_Mod_ForName(char *name, qboolean crash)
{
	model_t	*mod;
	unsigned *buf;
	int		i;
	...
	if (!name[0])
		Com_Error (ERR_DROP, "SW_Mod_ForName: NULL name");
	...
	case IDALIASHEADER:
		...
		SW_Mod_LoadAliasModel(mod, buf);
		break;

	case IDSPRITEHEADER:
		loadmodel->extradata = Hunk_Begin (0x10000);
		SW_Mod_LoadSpriteModel(mod, buf);
		break;

	case IDBSPHEADER:
		loadmodel->extradata = Hunk_Begin (0x1000000);
		SW_Mod_LoadBrushModel(mod, buf);
		break;
...
===============
*/
mleaf_t *SW_Mod_PointInLeaf(vec3_t p, model_t *model)
{
	mnode_t		*node;

	if (!model || !model->nodes)
		Com_Error (ERR_DROP, "SW_Mod_PointInLeaf: bad model");
...
void SW_Mod_LoadBrushModel (model_t *mod, void *buffer)
{
	...
	if (i != BSPVERSION)
		Com_Error (ERR_DROP, "SW_Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
...
void RE_SW_BeginRegistration(char *model)
{
	...
	if ( strcmp(mod_known[0].name, fullname) || flushmap->value)
		SW_Mod_Free (&mod_known[0]);
	r_worldmodel = RE_SW_RegisterModel (fullname);
	R_NewMap ();
}
