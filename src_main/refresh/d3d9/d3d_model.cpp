...
float Mod_PlaneDot (float *org, cplane_t *plane)
{
	...
	else return DotProduct (org, plane->normal) - plane->dist;
}
...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	for (i = 0; i < count; i++, in++, out++)
	{
		...
		out->image = D3D9_FindImage (name, it_wall);

		if (!out->image)
		{
			Com_Error (PRINT_ALL, "Couldn't load %s\n", name);
...
void Mod_LoadFaces (lump_t *l)
{
	...
	// build as a separate pass so that we can sort the surfs by texture before building
	D3D9_BeginBuildingLightmaps (loadmodel);
	out = loadmodel->surfaces;
	loadmodel->numsurfaceverts = 0;

	for (surfnum = 0; surfnum < loadmodel->numsurfaces; surfnum++, out++)
	{
		out->firstvertex = loadmodel->numsurfaceverts;
		loadmodel->numsurfaceverts += out->numedges;

		// create lightmaps and polygons
		if (!(out->texinfo->flags & (SURF_SKY | SURF_TRANS33 | SURF_TRANS66 | SURF_WARP)))
			D3D9_CreateSurfaceLightmap (out);

		D3D9_BuildPolygonFromSurface (loadmodel, out);
	}

	D3D9_EndBuildingLightmaps (loadmodel);
	D3D9_BuildSurfaceVertexBuffer (loadmodel);
}
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	if (pheader->skinheight > MAX_LBM_HEIGHT) Com_Error (ERR_DROP, "model %s has a skin taller than %d", mod->name, MAX_LBM_HEIGHT);
	if (pheader->num_xyz <= 0) Com_Error (ERR_DROP, "model %s has no vertices", mod->name);
	if (pheader->num_xyz > MAX_VERTS) Com_Error (ERR_DROP, "model %s has too many vertices", mod->name);
	if (pheader->num_st <= 0) Com_Error (ERR_DROP, "model %s has no st vertices", mod->name);
	...
	{
		mod->skins[i] = D3D9_FindImage ((char *) pheader + pheader->ofs_skins + i * MAX_SKINNAME, it_skin);
	}
	...
	R_CreateAliasBuffers (mod, pheader);
}
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	for (i = 0; i < sprout->numframes; i++)
	{
		...
		mod->skins[i] = D3D9_FindImage (sprout->frames[i].name, it_sprite);
	}

	R_CreateSpriteBuffers (mod, sprout);
	mod->type = mod_sprite;
}
...
struct model_s *D3D9_RegisterModel (char *name)
{
	...
	if (mod)
	{
		...
		else if (mod->type == mod_alias)
		{
			...
			// ensure the buffers are kept alive
			R_CreateAliasBuffers (mod, pheader);
		}
