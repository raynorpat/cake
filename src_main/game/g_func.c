...
void door_blocked  (edict_t *self, edict_t *other)
{
	...
	if (!(other->svflags & SVF_MONSTER) && (!other->client) )
	{
		...
		if (other)
		{
			VectorMA (other->absmin, 0.5, other->size, other->s.origin); // HACK: for entity without their origin near the model
			BecomeExplosion1 (other);
		}
		return;
	}
...
void train_blocked (edict_t *self, edict_t *other)
{
	...
	if (!(other->svflags & SVF_MONSTER) && (!other->client) )
	{
		...
		if (other)
		{
			VectorMA (other->absmin, 0.5, other->size, other->s.origin); // HACK: for entity without an origin near the model
			BecomeExplosion1 (other);
		}
...
void door_secret_blocked  (edict_t *self, edict_t *other)
{
	...
	if (!(other->svflags & SVF_MONSTER) && (!other->client) )
	{
		...
		if (other)
		{
			VectorMA (other->absmin, 0.5, other->size, other->s.origin); // HACK: for entities without their origin near the model
			BecomeExplosion1 (other);
		}
		return;
	}
...
void use_killbox (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	// HACK: make sure that really everything is killed
	self->count--;
