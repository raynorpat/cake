...
unsigned char *enctype1_encoder(unsigned char *id, unsigned char *, int *);
...
void func6e(unsigned char *data, int len);
int  func7(int);
int  func7e(int len);
...
/* FS: Not sure if this will work.  Re-adapated from enctypeX */
    // L00429D10
unsigned char *enctype1_encoder(unsigned char *id, unsigned char *data, int *datalen) {
    unsigned int    tbuff[326];
    int             i,
                    len,
                    tmplen;
...
int enctype1encoder_wrapper(unsigned char *key, unsigned char *data, int size) {
    ...
    p = enctype1_encoder(key, data, &size);
    memmove(data, p, size);
	printf ("data2: %s\n", data);
