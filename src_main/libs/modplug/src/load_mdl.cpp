...
//---------------------------------------------------------------
{
	...
	while (dwMemPos+6 < dwMemLength)
	{
		...
		block = pp[0] | (pp[1] << 8);	pp += 2;
		blocklen = pp[0] | (pp[1] << 8) | (pp[2] << 16) | (pp[3] << 24);
		dwMemPos += 6;
		...
		case 0x454D:
			if (blocklen)
			{
				...
				m_lpszSongComments = (LPSTR) malloc(blocklen);
		...
		case 0x4150:
			...
			for (i=0; i<npatterns; i++)
			{
				...
				if (pmsh->version == 0) {
					...
					const MDLPATTERNDATA *pmpd = (MDLPATTERNDATA *)(lpStream + dwPos);
				...
				{
					patterntracks[i*32+j] = data[0];
					patterntracks[i*32+j] |= data[1] << 8;
				}
			}
			break;
		// TR: Track Data
		...
		case 0x4949:
			...
			for (i=0; i<ninstruments; i++)
			{
				...
				{
					UINT note = 12;
					if (!(Headers[nins] = (INSTRUMENTHEADER *) calloc(1, sizeof(INSTRUMENTHEADER)))) break;
			...
			{
				Headers[j] = (INSTRUMENTHEADER *) calloc(1, sizeof(INSTRUMENTHEADER));
			}
			break;
		// VE: Volume Envelope
		case 0x4556:
		...
		case 0x4153:
			...
			for (i=1; i<=m_nSamples; i++) if ((Ins[i].nLength) && (!Ins[i].pSample) && (smpinfo[i] != 3) && (dwPos < dwMemLength))
			{
				...
				{
					DWORD dwLen = pp[0] | (pp[1] << 8) | (pp[2] << 16) | (pp[3] << 24);
					dwPos += 4;
...
WORD MDLReadBits(DWORD &bitbuf, UINT &bitnum, LPBYTE &ibuf, UINT &bytesLeft, BYTE n)
//-----------------------------------------------------------------
{
