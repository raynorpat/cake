...
extern WORD MDLReadBits(DWORD &bitbuf, UINT &bitnum, LPBYTE &ibuf, UINT &bytesLeft, BYTE n);
...
//------------------------
{
	...
	{
		free(m_lpszPatternNames);
		m_lpszPatternNames = NULL;
	}
	if (m_lpszSongComments)
	{
		free(m_lpszSongComments);
		m_lpszSongComments = NULL;
	}
	...
	for (i=0; i<MAX_MIXPLUGINS; i++)
	{
		if ((m_MixPlugins[i].nPluginDataSize) && (m_MixPlugins[i].pPluginData))
		{
			...
			free(m_MixPlugins[i].pPluginData);
...
//------------------------------------------------------------
{
	return (MODCOMMAND *) calloc(1, rows*nchns*sizeof(MODCOMMAND));
}
	...
	case RS_MDL16:
		...
		if (len >= 4)
		{
			...
			for (UINT j=0; j<pIns->nLength; j++)
			{
				BYTE hibyte;
				BYTE sign;
				if (nFlags == RS_MDL16) lowbyte = (BYTE)MDLReadBits(bitbuf, bitnum, ibuf, len, 8);
				sign = (BYTE)MDLReadBits(bitbuf, bitnum, ibuf, len, 1);
				if (MDLReadBits(bitbuf, bitnum, ibuf, len, 1))
				{
					hibyte = (BYTE)MDLReadBits(bitbuf, bitnum, ibuf, len, 3);
				} else
				{
					hibyte = 8;
					while (!MDLReadBits(bitbuf, bitnum, ibuf, len, 1)) hibyte += 0x10;
					hibyte += MDLReadBits(bitbuf, bitnum, ibuf, len, 4);
				}
