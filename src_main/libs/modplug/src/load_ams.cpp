...
//-----------------------------------------------------------
{
	...
	// Read Pattern Names
	m_lpszPatternNames = (LPSTR) calloc(1, pfh->patterns * 32);
	...
	if (tmp)
	{
		m_lpszSongComments = (LPSTR) calloc(1, tmp + 1);
...
//------------------------------------------------------------
{
	...
	for (UINT nIns=1; nIns<=m_nInstruments; nIns++)
	{
		...
		INSTRUMENTHEADER *penv = (INSTRUMENTHEADER *) calloc(1, sizeof(INSTRUMENTHEADER));
		if (!penv) return TRUE;
	...
	// Comments
	{
		...
		if (composernamelen)
		{
			m_lpszSongComments = (LPSTR) malloc(composernamelen + 1);
...
{
	UINT tmplen = dmax;
	signed char *amstmp = (signed char *) malloc(tmplen);
