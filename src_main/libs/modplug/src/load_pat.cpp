...
BOOL PAT_Load_Instruments(void *c)
{
	...
	for(t=1; t<of->m_nInstruments; t++) { // xmms modplug doesn't use slot zero
		if(!(of->Headers[t] = (INSTRUMENTHEADER *) calloc(1, sizeof(INSTRUMENTHEADER)))) return FALSE;
	...
	if(!(of->Headers[0] = (INSTRUMENTHEADER *) malloc(sizeof(INSTRUMENTHEADER)))) return FALSE;
...
BOOL CSoundFile::ReadPAT(const BYTE *lpStream, DWORD dwMemLength)
{
	...
	for(t=1; t<(int)m_nInstruments; t++) { // xmms modplug doesn't use slot zero
		WaveHeader hw;
		char s[32];
		if(!(d = (INSTRUMENTHEADER *) calloc(1, sizeof(INSTRUMENTHEADER)))) {
			avoid_reentry = 0;
			return FALSE;
		}
		Headers[t] = d;
	...
	// copy last of the mohicans to entry 0 for XMMS modinfo to work....
	t = m_nInstruments - 1;
	if(!(Headers[0] = (INSTRUMENTHEADER *) malloc(sizeof(INSTRUMENTHEADER)))) {
		avoid_reentry = 0;
		return FALSE;
	}
