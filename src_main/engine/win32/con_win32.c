...
// cmd history
static char qconsole_history[QCONSOLE_HISTORY][MAXCMDLINE];
static int qconsole_history_pos = -1;
static int qconsole_history_oldest = 0;

// current edit buffer
static char qconsole_line[MAXCMDLINE];
...
{
	Q_strlcpy(qconsole_history[qconsole_history_oldest], qconsole_line, sizeof(qconsole_history[qconsole_history_oldest]));
...
static void CON_HistPrev (void)
{
	...
	Q_strlcpy(qconsole_line, qconsole_history[qconsole_history_pos], sizeof(qconsole_line));
...
void CON_Show (void)
{
	...
	COORD writeSize = { MAXCMDLINE, 1 };
	COORD writePos = { 0, 0 };
	SMALL_RECT writeArea = { 0, 0, 0, 0 };
	int i;
	CHAR_INFO line[MAXCMDLINE];
	WORD attrib;
...
=================
*/
void CON_WindowsColorPrint(char *msg)
{
