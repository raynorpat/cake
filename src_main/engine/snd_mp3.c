...
static int mp3_inputdata(snd_stream_t *stream)
{
	...
	bytes_read = fread(p->mp3_buffer + remaining, 1, MP3_BUFFER_SIZE - remaining, stream->fh.file);
	if (bytes_read == 0)
	{
		return -1;
	}
...
static int mp3_startread(snd_stream_t *stream)
{
	 ...
	 */
	ReadSize = fread(p->mp3_buffer, 1, MP3_BUFFER_SIZE, stream->fh.file);
	if (ReadSize != MP3_BUFFER_SIZE)
	{
		if (FS_FEof((fileHandle_t)stream->fh.file) || FS_FError((fileHandle_t)stream->fh.file))
			return -1;
	}
	...
	if (p->Stream.error)
	{
		Com_Printf("MP3: No valid MP3 frame found\n");
		return -1;
	}
	...
	case MAD_MODE_STEREO:
		...
		break;
	default:
		Com_Printf("MP3: Cannot determine number of channels\n");
		return -1;
	}
	...
	do
	{
		...
		if (mad_frame_decode(&p->Frame, &p->Stream))
		{
			...
			else
			{
				...
				{
					Com_Printf("MP3: unrecoverable frame level error (%s)\n", mad_stream_errorstr(&p->Stream));
					break;
				}
			}
		}
...
static snd_stream_t *S_MP3_CodecOpenStream (char *filename)
{
	snd_stream_t *stream;
	...
	{
		Com_Printf("%s is not a valid MP3 file.\n", filename);
		Z_Free(stream->priv);
...
static int S_MP3_CodecRewindStream (snd_stream_t *stream)
{
	...
	mp3_stopread(stream);
	FS_Rewind((fileHandle_t)stream->fh.file, (int)stream->fh.start, (int *)stream->fh.pos);
